...
public class ChessBoardRenderer {
		...
		switch(piece.getType()) {
			...
			case KNIGHT:
				//gc.fillOval(minX, minY, width, height);
                                ...
                                gc.fillRect(minX, minY + height * 0.8, width, height * 0.5);
				break;
			case PAWN: 
				drawTriangle(gc, minX, minY + height * 0.3, width, height * 0.7);
				break;
	...
	void drawAvailablePositions(Canvas canvas,ChessPiece aPiece) {
            PiecePosition[] positions = aPiece.getAvailablePositions(board);
            for (int i = 0; i < positions.length; i++) {
                drawSquareAt(canvas, positions[i].getColumn(), positions[i].getRow(), Color.CORAL);
            }
	}
